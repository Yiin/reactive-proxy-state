...
import { wrapState, watchEffect } from '../src/index';
...
describe("WatchEffect Tests", () => {
    ...
    test("watchEffect with tracking events", () => {
        const { events, emit } = createEventCollector();
        const state = wrapState({ count: 0 }, emit);
        const onTrack = mock((event) => { });
        const onTrigger = mock((event) => { });
    ...
    test('watchEffect does not trigger excessively for no-op mutations', () => {
        ...
        const setEffect = mock(() => { for (const _ of state.set)
            ; }); // Iterate to track
    ...
    test('watchEffect tracks getters', () => {
        ...
        const state = wrapState(stateObj, () => { });
        let value; // Keep value for later checks if needed, but don't assign in effect
        const effectFn = mock(() => {
