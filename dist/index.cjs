...
__export(exports_src, {
  ...
  trigger: () => trigger,
  trackVueReactiveEvents: () => trackVueReactiveEvents,
  track: () => track,
  toRefs: () => toRefs,
  toRef: () => toRef,
  toRaw: () => toRaw,
...
function trackVueReactiveEvents(vueState, emit, options = {}) {
  ...
  const stop = import_vue.watch(() => traverse(vueState), () => {
    try {
  ...
  function diffAndEmit(curr, old, basePath) {
    ...
    if (isObject4(curr) && isObject4(old)) {
  ...
  function diffObject(curr, old, basePath) {
  ...
  function diffArray(curr, old, basePath) {
  ...
  function diffMap(curr, old, basePath) {
  ...
  function diffSet(curr, old, basePath) {
    for (const v of old.values()) {
      if (!curr.has(v))
        emit({ action: "set-delete", path: basePath, value: v, oldValue: v });
    }
    for (const v of curr.values()) {
      if (!old.has(v))
        emit({ action: "set-add", path: basePath, value: deepClone(v) });
    }
  }
  return stop;
}
