...
function runBenchmark(name, setupFn, taskFn) {
...
const plainResults = runBenchmark("Plain Object Mutations", () => createComplexEntityState(), // Setup: create plain object
(state) => applyMutations(state) // Task: apply mutations
);
// Benchmark Wrapped Object
let collectedEvents = []; // Keep events outside to avoid impacting timing too much
const emit = (event) => {
